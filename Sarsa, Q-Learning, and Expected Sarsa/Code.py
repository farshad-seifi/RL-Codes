...
class cliff_walking:
    ...
    def Q_value_updator_expected_sarsa(self, current_state, reward, selected_action):
        x = current_state[0]
        y = current_state[1]
        Expected_Q = 0

        max_action = self.select_action_max()
        max_action_num = self.actions_list.index(max_action)
        next_state = self.next_state(max_action_num)
        next_x = next_state[0]
        next_y = next_state[1]
        Expected_Q += (1-self.epsilon) * self.Q_values[max_action_num][next_x][next_y]

        actions = self.actions_list.copy()
        actions.remove(max_action)

        for i in range(0, len(actions)):
            action = actions[i]
            action_num = self.actions_list.index(action)
            next_state = self.next_state(action_num)
            next_x = next_state[0]
            next_y = next_state[1]
            Expected_Q += (1/3) * (self.epsilon) * self.Q_values[action_num][next_x][next_y]

        selected_action_num = self.actions_list.index(selected_action)

        self.Q_values[selected_action_num][x][y] = self.Q_values[selected_action_num][x][y] +\
                                          self.alpha * (reward + self.gamma * Expected_Q - self.Q_values[selected_action_num][x][y])
...
simulations = 15000
episode_count_expected_sara = pd.DataFrame(np.zeros((simulations,2)))
w = cliff_walking()
Total_Reward = 0
Reward_Expected_Sarsa = pd.DataFrame(np.zeros((500, 10)))
...
for j in range(0, 10):
    ...
    for i in range(0, simulations):
        ...
        if is_terminate_state:
            w.reset_game()
            episode_count_expected_sara[0].iloc[i] = episode_count_expected_sara[0].iloc[i - 1] + 1
            episode_count_expected_sara[1].iloc[i] = reward
            Total_Reward = 0
            w.Q_values[w.actions_list.index(action)][current_state[0]][current_state[1]] = reward * w.gamma
        else:
            w.current_state = next_state
            # next_action = w.select_action()
            w.Q_value_updator_expected_sarsa(current_state, reward, action)
            if i >= 1:
                episode_count_expected_sara[0].iloc[i] = episode_count_expected_sara[0].iloc[i - 1]
        if action == 'u':
            u += 1
        elif action =='d':
            d +=1
            ...
            l += 1
    episode_count_expected_sara = episode_count_expected_sara[episode_count_expected_sara[1] != 0]
    episode_count_expected_sara.reset_index(inplace=True, drop=True)
    episode_count_expected_sara[2] = 0
    for o in range(1, len(episode_count_expected_sara) + 1):
        episode_count_expected_sara[2].iloc[o - 1] = episode_count_expected_sara[1].iloc[:o].mean()
    Reward_Expected_Sarsa[j] = episode_count_expected_sara[2].iloc[:500]


R_Q = Reward_Q_Learning.mean(axis= 1)
R_S = Reward_Sarsa.mean(axis= 1)
R_ES = Reward_Expected_Sarsa.mean(axis= 1)
...
plt.plot(R_ES.index, R_ES, color='green', label="Expected Sarsa")


# Adding labels and title
plt.xlabel('Episode')
plt.ylabel('Average Reward')
plt.title('Results of Sarsa, Q-Learning, and Expected Sarsa applied to the cliff-walking task (Ten simulations)')
